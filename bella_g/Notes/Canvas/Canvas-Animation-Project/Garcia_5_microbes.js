...
function updateMicrobes(anim, microbes){
	...
	for (var i = 0; i < microbes.length; i++)  {
		...
		/*
			...
			* 10fps -> 10
			* 
			* for a linear relationshipp, we can use the equation:
		...
		for (var n = 0; n < numNewSegmentsPerFrame; n++) {
			...
			// -0.1 to 0.1
			var newHeadAngle = headAngle + ((angleVariance / 2) - Math.random() *
			angleVariance);
			...
			if (newHeadX >= canvas.width || newHeadX <= 0 || newHeadY >= canvas.height
			|| newHeadY <= 0) {
